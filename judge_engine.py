...
class Judge:
    ...
    def handle_move(self, move, player_num):
        """Validate and execute a move. Returns 'forfeit', True, or False"""
        ...
        print(f"Player {player_num}'s move: {direction_str}{' (BOOST)' if use_boost else ''}")
        
        # Record move in game string
        move_abbrev = {'UP': 'U', 'DOWN': 'D', 'LEFT': 'L', 'RIGHT': 'R'}
        boost_marker = 'B' if use_boost else ''
        self.game_str += f"{player_num}{move_abbrev[direction_str]}{boost_marker}-"
        
        return (True, use_boost)  # Return tuple: (valid, boost_flag)
    ...
    while True:
        ...
        if not p1_move or not validation:
            if p1_random > 0:
                ...
                p1_random -= 1
                judge.game_str += "1R-"  # Mark as random
            else:
            ...
            # Convert validated move string to Direction
            direction_map = {
                'UP': Direction.UP, 'DOWN': Direction.DOWN,
                'LEFT': Direction.LEFT, 'RIGHT': Direction.RIGHT,
            }
            p1_direction = direction_map[p1_move.upper().split(':')[0]]
        ...
        if not p2_move or not validation:
            if p2_random > 0:
                ...
                p2_random -= 1
                judge.game_str += "2R-"  # Mark as random
            else:
            ...
            p2_direction = direction_map[p2_move.upper().split(':')[0]]
