...
class GameBoard:
    def __init__(self, height: int = 18, width: int = 20):
        self.height = height
        self.width = width
        # Create a 2D list representing the grid, initialized to EMPTY.
        self.grid = [[EMPTY for _ in range(width)] for _ in range(height)]

    # Applies torus (wraparound) logic to a given position.
    ...
    # Returns cell state post-torus_check
    ...
    # Random non-occupied cell to spawn clue on
    def get_random_empty_cell(self) -> tuple[int, int] | None:
        ...
        if not empty_cells:
            return None # No empty cells left on the board
...
# Using tuples for direction vectors makes movement calculations clean and simple.
UP = (0, -1)
DOWN = (0, 1)
RIGHT = (1, 0)
LEFT = (-1, 0)
...
class Agent:
    ...
    def __init__(self, agent_id: str, start_pos: tuple[int, int], start_dir: Direction, board: GameBoard):
        ...
        # Deque structure : [(start).....(head)]
        # compute second cell from start_dir.value (enum values are tuples)
        ...
        # Place the initial agent position on the board
    ...
    def move(self, direction: Direction, other_agent: Optional['Agent'] = None, use_boost: bool = False) -> bool:
        ...
        for move_num in range(num_moves):
            # Prevent reversing directly into ourselves: ignore exact-opposite direction
            ...
            if cell_state == AGENT:
                ...
                if other_agent and other_agent.alive and new_head in other_agent.trail:
                    ...
                    if other_agent.is_head(new_head):
                        # Head-on collision: longer trail survives
                        if self.length > other_agent.length:
                            # We survive, other agent dies
                            other_agent.alive = False
                            # Add new head (leave trail)
                            self.trail.append(new_head)
                            self.length += 1
                            self.board.set_cell_state(new_head, AGENT)
                            continue  # Continue to next move if boosting
                        else:
                            # We die (equal length or shorter)
                            self.alive = False
                            if self.length == other_agent.length: # both die on equal length
                                other_agent.alive = False
                            return False
                    else:
